import XCTest
@testable import LadderLog

final class LadderLogTests: XCTestCase {
    var store: Store!
    ...
    @MainActor func testAddEntrySucceedsUnderLimit() {
        ...
        let added = store.add(LadderEntry(), isPro: false)
        XCTAssertTrue(added)
        XCTAssertEqual(store.entries.count, before + 1)
    }

    @MainActor func testAddEntryFailsAtFreeLimit() {
        store.entries = (0..<Store.freeLimit).map { _ in LadderEntry() }
        let added = store.add(LadderEntry(), isPro: false)
    ...
    @MainActor func testProUserBypassesLimit() {
        store.entries = (0..<Store.freeLimit).map { _ in LadderEntry() }
        let added = store.add(LadderEntry(), isPro: true)
        XCTAssertTrue(added)
    }
    ...
    @MainActor func testDeleteAtOffsets() {
        store.entries = [LadderEntry(), LadderEntry()]
